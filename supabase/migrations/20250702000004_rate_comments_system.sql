-- Rate Comments System Migration
...
CREATE TABLE rate_submission_comments (
  ...
  -- Constraints
  CONSTRAINT valid_reply_depth CHECK (
    parent_comment_id IS NULL OR 
    (SELECT parent_comment_id FROM rate_submission_comments WHERE id = parent_comment_id) IS NULL
  ), -- Only allow 2 levels: comment -> reply (no nested replies)
  
  CONSTRAINT valid_target_reference CHECK (
    (target_type = 'rate_submission' AND EXISTS (SELECT 1 FROM carrier_rate_submissions WHERE id = target_id)) OR
    (target_type = 'insurance_info' AND EXISTS (SELECT 1 FROM carrier_insurance_info WHERE id = target_id)) OR
    target_type IN ('carrier_general', 'safety_concern')
  )
);
